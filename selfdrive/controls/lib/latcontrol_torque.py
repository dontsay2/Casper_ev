...
class LatControlTorque(LatControl):
  ...
  def update(self, active, CS, VM, params, steer_limited, desired_curvature_ff, desired_curvature_now, llk, model_data=None):
    self.frame += 1
    if self.frame % 10 == 0:
    ...
    if not active:
      ...
      low_speed_factor = np.interp(CS.vEgo, LOW_SPEED_X, LOW_SPEED_Y if not self.use_nnff else LOW_SPEED_Y_NN)**2
      ...
      if model_good and (self.use_nnff or self.use_nnff_lite):
        ...
        desired_lateral_jerk = (np.interp(self.desired_lat_jerk_time, ModelConstants.T_IDXS, model_data.acceleration.y) - desired_lateral_accel_now) / self.desired_lat_jerk_time
